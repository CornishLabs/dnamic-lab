...
class Fluoresce(Fragment):
    ...
    def device_setup(self): # Run at the start of each run_once()
        ...
        # I actually think this only makes sense for a single tone on in an exp. In reality if these are 
        # changing, I should have this put in a seperate function. 
    ...
    @kernel
    def turn_light_off_now_keep_shutters(self):
        self.dds_ch_rb_cool.sw.off()
        self.dds_ch_rb_repump.sw.off()

    @kernel
    def turn_light_off_now_close_shutters(self):
        self.turn_light_off_now_keep_shutters()
...
class LoadRbMOT(Fragment):
    ...
    def load_mot_on(self):
        ...
        self.MOT_fluoresce.turn_light_on_now()

    @kernel
    def load_mot_off(self):
        self.MOT_quad.turn_off()
        self.MOT_fluoresce.turn_light_off_now_close_shutters()
    
class LoadRbMOTImageExp(ExpFragment):
    def build_fragment(self):
