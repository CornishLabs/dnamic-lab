"""Archived monolithic Cs MOT implementation, retained as a reference."""
...
class LoadCsMOTImageMonolith(ExpFragment):
    def build_fragment(self):
...
LoadCsMOTImageMonolithExp = make_fragment_prepared_dashboard_scan_exp(
    LoadCsMOTImageMonolith,
...
class LoadCsMOTToTweezersImageMonolith(ExpFragment):
    def build_fragment(self):
        self._live_imaging_initialised = False
...
LoadCsMOTToTweezersImageMonolithExp = make_fragment_prepared_dashboard_scan_exp(
    LoadCsMOTToTweezersImageMonolith,
    max_rtio_underflow_retries=0,
)


class LoadCsMOTToTweezersImageStatisticsMonolith(ExpFragment):
    """Repeat the tweezer-image shot and publish one probability point."""

    def build_fragment(self):
        self.setattr_fragment("shot", LoadCsMOTToTweezersImageMonolith, detached=True)
        self.shot: LoadCsMOTToTweezersImageMonolith
...
class LoadCsMOTToTweezersImageStatisticsMonolithDashboard(
    LoadCsMOTToTweezersImageStatisticsMonolith
):
    """Dashboard wrapper for archived monolithic Cs loading statistics."""
...
LoadCsMOTToTweezersImageStatisticsMonolithExp = (
    make_fragment_prepared_dashboard_scan_exp(
        LoadCsMOTToTweezersImageStatisticsMonolithDashboard,
        max_rtio_underflow_retries=0,
    )
)
